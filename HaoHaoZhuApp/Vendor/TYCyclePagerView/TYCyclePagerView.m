...
- (void)addCollectionView {
    ...
    UILabel *pageControl = [UILabel new];
    pageControl.font = [ZMFont defaultAppFontWithSize:10];
    ...
    pageControl.backgroundColor = [ZMColor colorWithHexString:@"#000000" alpha:0.3];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (_delegateFlags.didScrollFromIndexToNewIndex && !TYEqualIndexSection(_indexSection, indexSection)) {
        ...
        _pageControl.text = [NSString stringWithFormat:@"%ld / %ld",_indexSection.index + 1,self.numberOfItems];
    }
}
...
- (void)layoutSubviews {
    ...
    _pageControl.frame = CGRectMake(self.bounds.size.width - 50 - 20, self.bounds.size.height - 20 - 10, 50, 20);
